...
function renderBucket() {
  ...
  items.forEach(([key, item]) => {
    ...
    li.innerHTML = `
      <span class="item-check" data-key="${key}">${item.done ? "✅" : "⬜"}</span>
      <div class="item-body">
        <div class="item-title">${esc(item.text)}</div>
        <div class="item-meta">
          ${item.cat  ? `<span class="item-cat">${esc(item.cat)}</span>` : ""}
          ${item.prio ? `<span class="item-badge prio-${item.prio}">優先:${esc(item.prio)}</span>` : ""}
          ${item.urg  ? `<span class="item-badge urg-${item.urg}">緊急:${esc(item.urg)}</span>` : ""}
        </div>
      </div>
      <div class="item-actions">
        <button class="act-edit"  data-key="${key}">✏️</button>
        <button class="act-trash" data-key="${key}">🗑️</button>
      </div>`;
...
function renderVisit() {
  ...
  items.forEach(([key, item]) => {
    ...
    li.innerHTML = `
      <div class="item-body">
        <div class="item-title">${esc(item.text || item.title || item.place || "")}</div>
        ${item.date ? `<div class="item-cat">${esc(item.date)}</div>` : ""}
...
function renderTrash() {
  ...
  items.forEach(([key, item]) => {
    ...
    li.innerHTML = `
      <div class="item-body">
        <div class="item-title">${esc(item.text)}</div>
        ${item.cat ? `<div class="item-cat">${esc(item.cat)}</div>` : ""}
      </div>
      <div class="item-actions">
